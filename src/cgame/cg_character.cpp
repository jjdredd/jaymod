...
qboolean CG_RegisterCharacter( const char *characterFile, bg_character_t *character )
{
	...
	// Jaybird - Register Undressed Live-Player Media
	if( *characterDef.undressedModel ) {
		// Register Undressed Live-Player Model
		if( !(character->undressedModel = trap_R_RegisterModel( characterDef.undressedModel )) )
			CG_Printf( S_COLOR_YELLOW "WARNING: failed to register undressed live-player model '%s' referenced from '%s'\n", characterDef.undressedModel, characterFile );
 	}
