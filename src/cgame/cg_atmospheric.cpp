	...
	if( cg.time < cg_atmFx.baseEndTime )
	{
		...
		*num = cg_atmFx.baseDrops;
		...
		if( cg.time < cg_atmFx.gustStartTime )
		{
			...
			*num = cg_atmFx.baseDrops + (int)((cg_atmFx.gustDrops - cg_atmFx.baseDrops) * frac);
		}
...
static void CG_EP_ParseInts( char *intstr, int *i1, int *i2 )
{
	...
	if( *middleptr )
	{
		*middleptr++ = 0;
		*i1 = (int)atof( intstr );
		*i2 = (int)atof( middleptr );
	}
	else {
		*i1 = *i2 = (int)atof( intstr );
	}
}
...
void CG_EffectParse( const char *effectstr )
{
	...
	cg_atmFx.baseMinTime = (int)(1000 * bmin);
	cg_atmFx.baseMaxTime = (int)(1000 * bmax);
	cg_atmFx.changeMinTime = (int)(1000 * cmin);
	cg_atmFx.changeMaxTime = (int)(1000 * cmax);
	cg_atmFx.gustMinTime = (int)(1000 * gmin);
	cg_atmFx.gustMaxTime = (int)(1000 * gmax);
	cg_atmFx.baseDrops = (int)bdrop;
	cg_atmFx.gustDrops = (int)gdrop;
...
void CG_AddAtmosphericEffects()
{
	...
	max = cg_atmosphericEffects.value < 1 ? (int)(cg_atmosphericEffects.value * cg_atmFx.numDrops) : cg_atmFx.numDrops;
