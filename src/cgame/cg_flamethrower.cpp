...
void CG_FireFlameChunks( centity_t *cent, vec3_t origin, vec3_t angles, float speedScale, qboolean firing )
{
	...
	if (	(centInfo->lastClientFrame == cent->currentState.frame) &&
		...
		while (t <= cg.time) {
			...
			f->timeStart = t;
			f->timeEnd = t + int( FLAME_LIFETIME * (1.0f/(0.5f + 0.5f*speedScale)) );
			...
			if (!firing) {
				f->gravity = -150;
				f->blueLife = int( FLAME_BLUE_LIFE * 0.1f );
			} else {
				f->gravity = 0;
		...
		f->timeStart = cg.time;
		f->timeEnd = cg.time + int( FLAME_LIFETIME * (1.0f/(0.5f + 0.5f*speedScale)) );
		f->size = FLAME_START_SIZE * speedScale;
...
void CG_AddFlameToScene( flameChunk_t *fHead ) {
	...
	isClientFlame = (fHead == centFlameInfo[fHead->ownerCent].lastFlameChunk) ? qtrue : qfalse;
		...
		if (!f->ignitionOnly &&
			...
			while (fNext && !droppedTrail) {
				if ( (Distance( f->org, fNext->org ) < ( (0.1 + 0.9*f->lifeFrac) * f->size*0.35 ) )
					...
					&&	(fabs( (float)(f->timeStart - fNext->timeStart) ) < 100)
