...
void CG_NewClientInfo( int clientNum ) {
    ...
    v = Info_ValueForKey (configstring, "mu" );
    newInfo.muted = atoi(v);
...
#define	ANIM_SPEEDMAX_HIGH	20
	...
	while( cg.time > lf->frameTime && !done ) {
		...
		if (lf->oldFrameSnapshotTime < snapPosTime) {
			...
			lf->moveSpeed = int( Distance( snapOrg, lf->oldFramePos ) / serverDelta );
...
static void CG_PlayerAnimation( centity_t *cent, refEntity_t *body ) {
	...
	if( !(cent->currentState.eFlags & (EF_DEAD | EF_PLAYDEAD)) && cent->pe.legs.yawing ) {
		//CG_Printf("turn: %i\n", cg.time );
...
static void CG_AddPainTwitch( centity_t *cent, vec3_t torsoAngles ) {
	...
	if( cent->currentState.eFlags & (EF_DEAD |EF_PLAYDEAD) ) {
		cent->pe.painAnimLegs = -1;
		cent->pe.painAnimTorso = -1;
		cent->pe.animSpeed = 1.0;
		return;
...
static void CG_PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] ) {
	...
	if( cent->currentState.eFlags & (EF_DEAD | EF_PLAYDEAD) || cent->currentState.eFlags & EF_MOUNTEDTANK ) {
...
Jaybird - This has been modified to show all sprites for Shoutcasters
===============
*/
static void CG_PlayerSprites( centity_t *cent ) {
	...
	if ( cg_drawDisconnectIcon.integer && (cent->currentState.eFlags & EF_CONNECTION) ) {
		CG_PlayerFloatSprite( cent, cgs.media.disconnectShader, 56 );
		return;
	}

	if ( (cent->currentState.eFlags & EF_POISON) ) {
		if ((cent->currentState.number == cent->currentState.clientNum
			&& cg.snap->ps.stats[ STAT_PLAYER_CLASS ] == PC_MEDIC
			&& cg.snap->ps.stats[ STAT_HEALTH ] > 0
			&& cg.snap->ps.persistant[PERS_TEAM] == team ) ||
			(cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR
			&& cgs.clientinfo[cg.snap->ps.clientNum].shoutcaster )){
			CG_PlayerFloatSprite( cent, cgs.media.poisonShader, 56 );
			return;
		}
	}
	...
	if( (cent->currentState.eFlags & EF_DEAD) ) {
		if((cent->currentState.number == cent->currentState.clientNum && cg.snap->ps.stats[ STAT_PLAYER_CLASS ] == PC_MEDIC
			&& cg.snap->ps.stats[ STAT_HEALTH ] > 0
			&& cg.snap->ps.persistant[PERS_TEAM] == team ) ||
			(cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR && cgs.clientinfo[cg.snap->ps.clientNum].shoutcaster )) {
...
/*
===============
CG_AddPlayerGlowEffects
===============
*/
void CG_AddPlayerGlowEffects( centity_t *cent ) {
	float r, g, b;
	int clientNum;
	clientInfo_t *ci;
	...
	if (ci->team != TEAM_ALLIES && ci->team != TEAM_AXIS)
		return;
	
	if ( !cvars::bg_glow.ivalue )
		return;
	...
	do {
		...
		// For the next trip around
		clientNum = clientNum >> 3;
	} while ((r + b + g) == 0.f);

	trap_R_AddLightToScene( cent->lerpOrigin, 400, 1.0, r, g, b, 0, REF_FORCE_DLIGHT );
...
void CG_Player( centity_t *cent )
{
	...
	refEntity_t		head;
	refEntity_t		acc;
	vec3_t			playerOrigin = { 0.0f, 0.0f, 0.0f };
	vec3_t			lightorigin;
	int				clientNum,i;
	int				renderfx;
	qboolean		shadow;
...
//	float			gumsflappin = 0;	// talking amplitude
	...
	// add the body
	//
	// Jaybird - added case for Live Uniform Stealing
	...
	} else if( CG_HasJayFlag( cent, 1, JF_LOSTPANTS )) {
		if( character->undressedModel )
			body.hModel	= character->undressedModel;
		else
			body.hModel	= character->undressedCorpseModel;
		body.customSkin	= character->undressedCorpseSkin;
	} else {
...
// DEBUG
	...
	if( cent->currentState.eType != ET_CORPSE && !(cent->currentState.eFlags & (EF_DEAD | EF_PLAYDEAD)) ) {
	...
	//
    // Jaybird - TODO: add playdead check?
	if( !(cent->currentState.eFlags & (EF_DEAD | EF_PLAYDEAD)) ) {			// NERVE - SMF
...
/*	refdef.fov_x = (int)((float)refdef.width / 640.0f * 90.0f);
	...
	if( cg.time >= pi->torso.frameTime ) {
		...
		while( cg.time >= pi->torso.frameTime ) {
			pi->torso.frameTime += pi->torso.animation->duration / pi->torso.animation->numFrames;
			pi->torso.frame++;
	...
	if( cg.time >= pi->legs.frameTime ) {
		...
		while( cg.time >= pi->legs.frameTime ) {
			pi->legs.frameTime += pi->legs.animation->duration / pi->legs.animation->numFrames;
			pi->legs.frame++;
...
qboolean CG_HasJayFlag( centity_t *cent, int set, int flag ) {
	if( set == 1 ) {
		if( cent->currentState.effect1Time & flag )
			return qtrue;
		if( cent->currentState.clientNum == cg.snap->ps.clientNum && ( cg.snap->ps.persistant[PERS_JAYFLAGS1] & flag ))
			return qtrue;
	}
	if( set == 2 ) {
		if( cent->currentState.effect2Time & flag )
			return qtrue;
		if( cent->currentState.clientNum == cg.snap->ps.clientNum && ( cg.snap->ps.persistant[PERS_JAYFLAGS2] & flag ))
			return qtrue;
	}
	return qfalse;
}
