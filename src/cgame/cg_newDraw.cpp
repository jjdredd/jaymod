...
void CG_DrawPlayerWeaponIcon( rectDef_t *rect, qboolean drawHighlighted, int align, vec4_t *refcolor ) {
	...
	if (realweap == WP_KNIFE && cg.pmext.a2_throwingKnife.state == A2_THROWINGKNIFE_ARMING) {
		float f;
		int delta = cg.time - cg.pmext.a2_throwingKnife.depressedTime;
		if (delta < 0)
			delta = 0;
		else if (delta > 1000)
			delta = 1000;
		...
		f = (float)delta;
		hcolor[0] = 1.0f - min(1.0f, (max(0.0f, f - 500.0f) / 500.0f));
		hcolor[1] = 1.0f;
		hcolor[2] = 1.0f - min(1.0f, f / 500.0f);
		...
		if (delta < 1000) {
			...
			// pulse
			scale = (((cg.time - cg.pmext.a2_throwingKnife.depressedTime) % 1000) + 1) / 100.0f;
			halfScale = scale * 0.5f;
		}
	}
    else if (realweap == WP_MOLOTOV && cg.pmext.a2_molotov.state == A2_MOLOTOV_ARMING) {
        float f;
        int delta = cg.time - cg.pmext.a2_molotov.depressedTime;
        if (delta < 0)
            delta = 0;
        else if (delta > 1000)
            delta = 1000;
        ...
        if (delta < 1000) {
            ...
            // pulse
            scale = (((cg.time - cg.pmext.a2_molotov.depressedTime) % 1000) + 1) / 100.0f;
            halfScale = scale * 0.5f;
        }
    }
	else if (cg.predictedPlayerState.grenadeTimeLeft) {
...
void CG_MouseEvent(int x, int y) {
	switch( cgs.eventHandling ) {
		...
		case CGAME_EVENT_FIRETEAMMSG:
			...
			if( (cgameEvent_e)cgs.eventHandling == CGAME_EVENT_SPEAKEREDITOR ) {
...
void CG_EventHandling( int type, qboolean fForced )
{
	...
	switch( type ) {
		...
		default:
			...
			if( (cgameEvent_e)cgs.eventHandling == CGAME_EVENT_GAMEVIEW ) {
			...
			} else if( (cgameEvent_e)cgs.eventHandling == CGAME_EVENT_SPEAKEREDITOR ) {
			...
			} else if( (cgameEvent_e)cgs.eventHandling == CGAME_EVENT_CAMPAIGNBREIFING ) {
				type = CGAME_EVENT_GAMEVIEW;
			} else if( (cgameEvent_e)cgs.eventHandling == CGAME_EVENT_FIRETEAMMSG ) {
