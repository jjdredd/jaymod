	...
	for ( e = 0; e < moveEntities; e++ ) {
		...
		*obstacle = check;
		...
		for ( p=pushed_p ; p>pushed ; p-- ) {
			...
			if ( work->ent->client ) {
				work->ent->client->ps.delta_angles[YAW] = int( work->deltayaw );
...
//----(SA)	numerous changes start here
		...
		case MOVER_1TO2ROTATE:	// opening
			...
			if(kicked) {
				...
				ent->s.apos.trDuration = int( ent->gDuration / 2.0f );
			} else if(soft) {
...
void InitMover( gentity_t *ent ) {
	...
	if(ent->closespeed) {
		...
		ent->gDurationBack = int( distance * 1000 / ent->closespeed );
...
void Reached_Train_rotating( gentity_t *ent ) {
	...
	if (next->duration)
		ent->s.pos.trDuration = int( next->duration * 1000 );
	else	
		ent->s.pos.trDuration = int( length * 1000 / speed );

	// Rotate the train
	frames = ent->s.pos.trDuration / 100;
...
//			ent->s.solid = CONTENTS_SOLID;	// FIXME: allow other contents?
	...
	ent->spawnDebug |= 1; // Jaybird - debugging
}
