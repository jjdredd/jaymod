...
static bool Q_ValidateIp(const char* ip)
{
    ...
    // Number of digits in the current segment
    int digits = 0;

    // Number of segments we've encountered
    int segments = 0;
    ...
    for ( ; *s; s++) {
        ...
        // Check for numbers
        if (*s >= '0' && *s <= '9') {
            // Check for too many digits in one octet
            if (digits == 3) {
                return false;
            }
            ...
            // Add the number to the segment value
            val = val * 10 + num;

            // Increment the number of digits in the segment
            digits++;
        ...
        // Check for .
        if (*s == '.') {
            // Make sure we don't encounter a dot after the 3rd segment
            if (segments == 3) {
                return false;
            }
        ...
        // If we reached here, an invalid character was reached
        return false;
    }

    // Out of the loop, we should have 3 segments, with the 4th in progress
    if (segments < 3) {
        return false;
    }
    ...
    // Make sure we're starting a port number
    if (*s != ':') {
		return false;
	}
...
qboolean Info_Validate( const char *s ) {
	...
	// Semi-colons aren't allowed
	if ( strchr( s, ';' ) ) {
		return qfalse;
	}

	// New lines aren't allowed
	if (strchr(s, '\n')) {
		return qfalse;
	}

	// Beginning MUST start with backslash
	if (s[0] != '\\') {
		return qfalse;
	}

	// End must NOT end with backslash
	if (s[strlen(s) - 1] == '\\') {
		return qfalse;
	}

	// Copy the userinfo into another string so we can tokenize and butcher it
	char str[MAX_INFO_STRING];
	memcpy(str, s, MAX_INFO_STRING);

	// Make sure there are an even number of tokens and no key duplicates (avoids injected ip addresses)
	map<string, string> keys;

	// Get the first token
	char* ptr = strtok((char*)str, "\\");

	// Make sure we got a first token
	if (ptr == NULL) {
		return qfalse;
	}
	...
	ptr = Info_ValueForKey(s, "name");
	if (strlen(ptr) == 0) {
		return qfalse;
	}

	// Make sure that it doesn't contain extended ASCII
	for (int i = 0, end = strlen(ptr); i < end; i++) {
		char c = ptr[i];
	...
	if (!Q_ValidateIp(Info_ValueForKey(s, "ip"))) {
		return qfalse;
	}
